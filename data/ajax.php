...
switch ($action) {
    case 'tuteur':
        $idEnt = $_POST['idEnt'];
        $sql = "SELECT * FROM sta_contact WHERE identreprise=".$idEnt;
        ...
        <select name='idContact' class="form-control" id="selectTuteur">
            <?php while ($ligne = $q->fetch()) {
                $nomTuteur = $ligne["nom"]." ".$ligne["prenom"];
                echo "<option value=".$ligne["idcontact"].">".$nomTuteur."</option>";
                // echo "<option value=" . $ligne[0] . ">" . $ligne[1] . "</option>";
            } ?>
        </select>
        <a data-toggle="modal" data-target="#ajoutTuteurDepuisStage" class="btn btn-primary" style="color: white"><i
                    class="fas fa-plus"></i>Ajouter un nouveau tuteur</a>
        </div>
        <?php
    break;
